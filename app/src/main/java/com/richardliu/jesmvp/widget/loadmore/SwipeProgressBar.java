...
public class SwipeProgressBar extends android.support.v7.widget.AppCompatImageView {
    ...
    public SwipeProgressBar(Context context) {
        super(context);
        init(context, null, 0);

    }

    public SwipeProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);

    }

    public SwipeProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.SwipeProgressBar, defStyleAttr, 0);
        final float density = getContext().getResources().getDisplayMetrics().density;

        mBackGroundColor = a.getColor(
                R.styleable.SwipeProgressBar_swipepb_background_color, DEFAULT_CIRCLE_BG_LIGHT);

        mProgressColor = a.getColor(
                R.styleable.SwipeProgressBar_swipepb_progress_color, DEFAULT_CIRCLE_BG_LIGHT);
        mColors = new int[]{mProgressColor};

        mInnerRadius = a.getDimensionPixelOffset(
                R.styleable.SwipeProgressBar_swipepb_inner_radius, -1);

        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.SwipeProgressBar_swipepb_progress_stoke_width, (int) (STROKE_WIDTH_LARGE * density));
        mArrowWidth = a.getDimensionPixelOffset(
                R.styleable.SwipeProgressBar_swipepb_arrow_width, -1);
        mArrowHeight = a.getDimensionPixelOffset(
                R.styleable.SwipeProgressBar_swipepb_arrow_height, -1);
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.SwipeProgressBar_swipepb_progress_text_size, (int) (DEFAULT_TEXT_SIZE * density));
        mTextColor = a.getColor(
                R.styleable.SwipeProgressBar_swipepb_progress_text_color, Color.BLACK);

        mShowArrow = a.getBoolean(R.styleable.SwipeProgressBar_swipepb_show_arrow, false);
        mCircleBackgroundEnabled = a.getBoolean(R.styleable.SwipeProgressBar_swipepb_enable_circle_background, true);
        mProgress = a.getInt(R.styleable.SwipeProgressBar_swipepb_progress, 0);
        mMax = a.getInt(R.styleable.SwipeProgressBar_swipepb_max, 100);
        int textVisible = a.getInt(R.styleable.SwipeProgressBar_swipepb_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
    ...
    private class OvalShadow extends OvalShape {
        ...
        public void draw(Canvas canvas, Paint paint) {
            final int viewWidth = SwipeProgressBar.this.getWidth();
            final int viewHeight = SwipeProgressBar.this.getHeight();
